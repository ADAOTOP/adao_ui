...
import { Button, Flex, Text, InjectedModalProps } from '@kaco/adao_ui';
...
const StartPage: React.FC<StartPageProps> = ({ goToApprove, goToChange, goToRemove, onDismiss }) => {
  ...
  /**
   * Check if the wallet has the required KAC allowance to change their profile pic or reactivate
      ...
      <Flex alignItems="center" style={{ height: '48px' }} justifyContent="center">
        <Text as="p" color="failure">
          {!hasMinimumCakeRequired &&
            t('%minimum% KAC required to change profile pic', { minimum: getFullDisplayBalance(minimumCakeRequired) })}
        </Text>
      </Flex>
      {profile.isActive ? (
        <>
