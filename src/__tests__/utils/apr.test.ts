...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { kacRewardsApr: cakeRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null);
    expect(cakeRewardsApr).toBeNull();
    expect(lpRewardsApr).toEqual(0);
  });
  it(`returns null when APR is infinite`, () => {
    const { kacRewardsApr: cakeRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '');
    expect(cakeRewardsApr).toBeNull();
    expect(lpRewardsApr).toEqual(0);
  });
  // it(`get the correct pool APR`, () => {
